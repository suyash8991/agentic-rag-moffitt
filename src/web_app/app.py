...
src_dir = current_file.parent.parent  # web_app/app.py -> web_app -> src
...
try:
    ...
    # This should be handled by the path setting above, but keep as fallback

    # Now try absolute imports
    from web_app.state.session import (
        init_session_state,
        get_current_page,
        set_current_page,
        add_user_message,
    ...
    from web_app.utils.styling import (
        set_page_config,
        apply_styles,
        format_title,
        format_subtitle,
    ...
    from web_app.components.sidebar import render_sidebar
    from web_app.components.chat import render_chat_interface
...
def render_settings():
    ...
    if st.button("Test LLM Connection"):
        with st.spinner("Testing LLM connection..."):
            try:
                from models.llm import get_llm_model, generate_text
